...
import { createConversionRetryKeyStore } from "../inquiry-ui-model.js"
...
export function InquiryDetailHost({ id }: { id: string }) {
  ...
  const conversionKeys = useMemo(() => createConversionRetryKeyStore(), [])
  ...
  return (
    <InquiryWorkspace
      ...
      onConvertToProposal={() =>
        conversionKeys.run("proposal", (idempotencyKey) =>
          mutations.convert.mutateAsync({ id, input: { kind: "proposal", idempotencyKey } }),
        )
      }
      onConvertToBookingSession={(targetLinkId) =>
        conversionKeys.run(`booking_session:${targetLinkId}`, (idempotencyKey) =>
          mutations.convert.mutateAsync({
            id,
            input: { kind: "booking_session", targetLinkId, idempotencyKey },
          }),
        )
      }
    />
